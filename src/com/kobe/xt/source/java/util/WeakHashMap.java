...
public class WeakHashMap<K,V>
    ...
    public V get(Object key) {
        ...
        // 遍历链表，找到了对应的元素就返回元素的 value
        while (e != null) {
            if (e.hash == h && eq(k, e.get()))
                return e.value;
            e = e.next;
        }
        // 没有找到对应的元素，那么就返回 null
        return null;
    }
    ...
    public V put(K key, V value) {
        // 校验 key 是否为空，如果为空那么使用 NULL_KEY（Object NULL_KEY = new Object()）对象代替；
        Object k = maskNull(key);
        // 计算 key 对应的哈希值
        int h = hash(k);
        // 获取 table 数组，在获取的时候会删除过期的Entry；
        Entry<K,V>[] tab = getTable();
        // 计算元素插入到 table 数组那个下标中
        int i = indexFor(h, tab.length);

        //遍历下标 i 对应的链表，看是否存在 key 和插入元素的 key 相同的元素
        for (Entry<K,V> e = tab[i]; e != null; e = e.next) {
            if (h == e.hash && eq(k, e.get())) {
                //存在 key 相同的元素
                V oldValue = e.value;
                if (value != oldValue)
                    //如果 value 不同，那么就将旧值替换为新插入元素的 value 值；
                    e.value = value;
                //返回旧值
                return oldValue;
            }
        ...
        modCount++;
        Entry<K,V> e = tab[i];
        // 如果下标 i 对应的链表中，没有找到 key 和插入元素的 key 相同的元素，那么就将插入的元素插入到链表的头部；
        tab[i] = new Entry<>(k, value, queue, h, e);
        // 如果插入元素之后 WeakHashMap 中元素的数量达到了扩容的阈值，那么就将 WeakHashMap 扩容为原来的两倍；
        if (++size >= threshold)
            resize(tab.length * 2);
        return null;
    }
    ...
    void resize(int newCapacity) {
        ...
        //根据传入的 newCapacity 创建对应长度的新数组；
        Entry<K,V>[] newTable = newTable(newCapacity);
        // 将旧数组中的元素移动到新数组中，如果旧数组中有失效的元素，那么就删除失效的元素
         ...
         */
        //如果元素的个数大于等于扩容阈值的一半，那么使用新的容量重新计算扩容阈值；
        if (size >= threshold / 2) {
            threshold = (int)(newCapacity * loadFactor);
        } else {
            //如果元素的个数小于扩容阈值的一半，那么就还是使用旧数组，因为在调用 transfer() 方法的时候，会删除失效的元素，所以元素的个数可能没有那么大，就不需要进行扩容了。
            expungeStaleEntries();
